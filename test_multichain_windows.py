"""
Windows-Compatible Multi-Chain Connectivity Test

Tests multi-chain functionality without Unicode characters that cause 
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/multichain_test_windows.log')
    ]
)
...
async def test_infura_client():
    ...
    try:
        ...
        async with InfuraClient(config) as infura_client:
            ...
            logger.info(f"Infura connectivity: {healthy_count}/{len(health_status)} chains healthy")
...
async def main():
    """Run all tests"""
    logger.info("Starting multi-chain connectivity tests (Windows compatible)...")
    ...
    if passed_count == total_count:
        ...
        print("\n" + "="*60)
        print("MULTI-CHAIN TEST RESULTS")
        print("="*60)
        print(f"Configuration: PASSED - Found {len(Config().chains)} chains")
        print("Infura Client: PASSED - All supported chains connected")
        print("Multi-Chain Client: PASSED - Cross-chain operations working")
        print("\nYour multi-chain blockchain analytics system is ready!")
        print("="*60)
        return True
    else:
